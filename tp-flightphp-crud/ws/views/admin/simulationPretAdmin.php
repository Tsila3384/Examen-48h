<div class="simulation-container">
    ...
    <div class="page-header">
        <h1>Simulateur de prêt (Admin)</h1>
        <p>Calculez les mensualités et le coût total pour un client</p>
    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-wrapper">
                            <select id="client_id" name="client_id" required>
                                <option value="" data-type-client-id="" disabled selected>Sélectionnez un client</option>
                                <?php foreach ($clients as $client): ?>
                                    <option value="<?= htmlspecialchars($client['id']) ?>" data-type-client-id="<?= htmlspecialchars($client['type_client_id']) ?>">
        ...
        <div class="result-container">
            <div id="simulationResult" class="simulation-result" style="display: none;">
                ...
                <div class="result-actions">
                    <a href="<?= BASE_URL ?>/admin/prets/formulairePret" class="btn-primary">
...
<script>
    ...
    // Loan types data from PHP
    const typesPret = <?= json_encode($typesPret) ?>;
    ...
    // Update loan type options based on selected client
    function updateTypePretOptions() {
        const selectedClient = clientSelect.options[clientSelect.selectedIndex];
        const typeClientId = selectedClient ? selectedClient.dataset.typeClientId : '';

        // Clear existing options
        typePretSelect.innerHTML = '<option value="" disabled selected>Sélectionnez un type de prêt</option>';

        if (typeClientId) {
            // Group types by id to avoid duplicates
            const uniqueTypes = {};
            typesPret.forEach(type => {
                if (!uniqueTypes[type.id] && type.type_client_id == typeClientId) {
            ...
            Object.values(uniqueTypes).forEach(type => {
                ...
                option.dataset.taux = type.taux_interet || 0;
                option.textContent = `${type.nom} (${type.taux_interet || 0}%)`;
    ...
    clientSelect.addEventListener('change', updateTypePretOptions);
    typePretSelect.addEventListener('change', updateTauxDisplay);

    // Initialize
    document.addEventListener('DOMContentLoaded', () => {
        updateTypePretOptions();
        updateTauxDisplay();
    });
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch('<?= BASE_URL ?>/admin/prets/simuler', {
                method: 'POST',
                headers: {
    ...
    document.getElementById('saveSimulation').addEventListener('click', async () => {
        ...
        try {
            const response = await fetch('<?= BASE_URL ?>/admin/prets/sauvegarderSimulation', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
